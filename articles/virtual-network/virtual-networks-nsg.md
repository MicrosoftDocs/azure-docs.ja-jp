...
> [!NOTE]
> 前述のルールの発信元アドレス範囲は**インターネット**であって、ロード バランサーの仮想 IP アドレスではありません。 発信元ポートは 50001 ではなく * です。 ロード バランサーの NAT 規則は、NSG セキュリティ規則とは異なります。 NSG セキュリティ規則は、常にトラフィックの最初の発信元と最終的な宛先に関連付けられ、両者の間にあるロード バランサーは**関係しません**。 Azure Load Balancer では、発信元の IP アドレスとポートが常に保持されます。
> 
> 
