...
レプリカやパーティションの割り当てを増やしたり変更したりする場合は、Azure Portal の使用をお勧めします。 ポータルでは、上限に達しないようにするための許容される組み合わせに強制的に制限されます。 スクリプト ベースまたはコード ベースのプロビジョニング方法が必要な場合は、代わりに [Azure PowerShell](search-manage-powershell.md) または[管理 REST API](https://docs.microsoft.com/rest/api/searchmanagement/services) を使用します。
...
2. **[設定]** で、 **[スケール]** ページを開いてレプリカとパーティションの数を変更します。 
   ...
   ![現在の値が表示された [スケール] ページ](media/search-capacity-planning/1-initial-values.png "現在の値が表示された [スケール] ページ")

3. スライダーを使ってパーティションの数を増減します。 下部の式は、使用される検索ユニットの数を示しています。
...
3. **[保存]** をクリックして変更を確認します。
   ...
   ![ポータルの状態メッセージ](media/search-capacity-planning/4-updating.png "ポータルの状態メッセージ")
...
[Azure Search の価格レベルの選択](search-sku-tier.md)
