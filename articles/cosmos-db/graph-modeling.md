...
description: Cosmos DB Gremlin API を使用してグラフ データベースをモデル化する方法について説明します。
author: luisbosquez
...
| Object | プロパティ | Type | メモ |
| --- | --- | --- |  --- |
| Vertex | id | string | パーティションごとに一意に適用されます。 挿入時に値が指定されないと、自動生成された GUID が格納されます。 |
...
エッジの方向を指定するには、`.addE()` Gremlin ステップに `.to()` または `.from()` 述語を使用します。 または、[Gremlin API 用 BulkExecutor ライブラリ](bulk-executor-graph-dotnet.md)を使用します。
