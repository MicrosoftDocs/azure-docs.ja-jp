...
2. **すべてのユーザー ドメインの Active Directory ドメイン コントローラーを AD FS サーバーと同じネットワークにデプロイする。**
    ...
    AD FS サーバーは、ユーザーの認証に Active Directory Domain Services を使用します。 AD FS サーバーと同じネットワークにドメイン コントローラーをデプロイするようお勧めします。 そうすることで、Azure ネットワークとオンプレミス ネットワークとの間でリンク障害が発生してもビジネスの継続性が保たれ、待機時間が短くなり、ログインのパフォーマンスが向上します。
