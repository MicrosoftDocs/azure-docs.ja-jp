...
場所は、ネットワークの場所のラベルであり、名前付きの場所または多要素認証の信頼できる IP を表します。

## <a name="named-locations"></a>名前付きの場所

名前付きの場所を使用すると、IP アドレス範囲または国や地域の論理グループを作成できます。

[条件付きアクセス] ページの **[管理]** セクションで、名前付きの場所にアクセスできます。

![条件付きアクセスでの名前付きの場所](./media/location-condition/02.png)

名前付きの場所には、次のコンポーネントがあります。

![新しい名前付きの場所を作成する](./media/location-condition/42.png)
...
- **不明な領域を含める** - IP アドレスによっては、特定の国またはリージョンにマップされないことがあります。 このオプションを使用すると、このような IP アドレスを名前付きの場所に含めるかどうかを選択できます。 名前付きの場所を使用するポリシーを不明な場所に適用する場合は、この設定を使用します。

構成できる名前付きの場所の数は、Azure AD 内の関連するオブジェクトのサイズによって制約されます。 組織は、それぞれに最大 1200 の IP 範囲が構成された、最大 90 個の名前付きの場所を構成できます。
...
「任意の場所」に適用するようにポリシーが構成されている場合は、IPv4 と IPv6 のトラフィックに適用されます。 指定した国と地域用に構成された名前付きの場所では、IPv4 アドレスのみがサポートされます。 IPv6 トラフィックが含まれるのは、「不明な領域を含める」オプションが選択されている場合のみです。
...
このオプションをオンにした後、名前付きの場所の **MFA の信頼できる IP** は、このオプションが選択されているすべてのポリシーに適用されます。
