...
| **ユーザーのセキュリティが侵害された (真陽性)** <br> "危険なユーザー" レポートには、危険なユーザー ([リスクの状態] = [危険]) が低い危険性 ([リスク レベル] = [低]) で表示されており、そのユーザーのセキュリティが実際に侵害された場合。 | ユーザーを選択し、[ユーザーに対するセキュリティ侵害を確認しますか?] をクリックします。 | Azure AD では、ユーザー リスクを "高" に変更し ([リスクの状態] = [セキュリティ侵害の確認済み]、[リスク レベル] = [高])、新しい検出 [ユーザーに対するセキュリティ侵害を管理者が確認しました] が追加されます。 | 現時点では、[ユーザーに対するセキュリティ侵害を確認しますか?] オプションは、"危険なユーザー" レポートでのみ使用できます。 <br> 検出 [ユーザーに対するセキュリティ侵害を管理者が確認しました] は、"危険なユーザー" レポートの [サインインにリンクされていないリスク イベント] タブに表示されます。 |
| **ユーザーが Azure AD Identity Protection の外部で修復された (真陽性 + 修復済み)** <br> "危険なユーザー" レポートには、危険なユーザーが表示されており、そのユーザーを後で Azure AD Identity Protection の外部で修復した場合。 | 1.ユーザーを選択し、[ユーザーに対するセキュリティ侵害を確認しますか?] をクリックします。 (このプロセスでは、ユーザーのセキュリティが実際に侵害されたことを Azure AD に確認します。) <br> 2.ユーザーの、[リスク レベル] が "高" になるまで待機します。 (この時間は、上記のフィードバックをリスク エンジンに取り込むために Azure AD が必要とする時間です。) <br> 手順 3.ユーザーを選択し、[ユーザーリスクを無視する] をクリックします。 (このプロセスでは、ユーザーのセキュリティが侵害されていないことを Azure AD に対して確認します。) |  Azure AD はユーザー リスクを "なし" に変更し ([リスクの状態] = [無視]、[リスク レベル] = [-])、アクティブなリスクがあるすべての既存のサインインに関するリスクをクローズします。 | [ユーザー リスクを無視する] をクリックすると、ユーザーおよび過去のサインインに対するすべてのリスクをクローズします。この削除操作は元に戻すことができません。 |
...
Identity Protection のユーザー リスク イベントに関するフィードバックはオフラインで処理され、更新に時間がかかる場合があります。 [リスク処理状態] 列にはフィードバック処理の現在の状態が表示されます。

!["危険なユーザー" レポートの [リスク処理状態]](./media/howto-provide-risk-event-feedback/risky-users-provide-feedback.png)

## <a name="next-steps"></a>次の手順

[Azure Active Directory Identity Protection のリスク イベント リファレンス](risk-events-reference.md)