...
1. ブループリント割り当て用のパラメーター値を指定します。
   ...
   - ブループリントのパラメーター
     ...
     - **共有サービスの場所**:成果物をデプロイする場所を決定します。 すべてのサービスがすべての場所で利用できるわけではありません。 このようなサービスをデプロイする成果物には、その成果物をデプロイする場所についてのパラメーター オプションが用意されています。
     - **Allowed location (Policy:Blueprint initiative for ISO 27001) (許可される場所 (ポリシー: ISO 27001 用ブループリント イニシアティブ))** :リソース グループとリソースに許可されている場所を示す値。
